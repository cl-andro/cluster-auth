...
public class PasswordEntry extends UUIDMap.Value {
    ...
    public PasswordEntry(UUID uuid) {
        super(uuid);
        _createdAt = System.currentTimeMillis();
        _updatedAt = _createdAt;
    }
    ...
    public static PasswordEntry fromJson(JSONObject obj) throws JSONException {
        UUID uuid;
        if (!obj.has("uuid")) {
        ...
        PasswordEntry entry = new PasswordEntry(uuid);
        entry.setTitle(obj.optString("title", ""));
        entry.setUsername(obj.optString("username", ""));
        entry.setPassword(obj.optString("password", ""));
    ...
    public void setPassword(String password) { _password = password; _updatedAt = System.currentTimeMillis(); }
