...
public class DatabaseImporterTest {
     ...
     *     -> while read line; do (qrencode "$line" -o - | feh -); done < ./app/src/test/resources/com/zk/cluster/auth/importers/plain.txt
    ...
    @Test
    public void testImportClusterPlain() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importPlain(ClusterImporter.class, "cluster_plain.json");
        checkImportedEntries(entries);
    }

    @Test
    public void testImportClusterEncrypted() throws IOException, DatabaseImporterException, OtpInfoException {
        List<VaultEntry> entries = importEncrypted(ClusterImporter.class, "cluster_encrypted.json", encryptedState -> {
            final char[] password = "test".toCharArray();
            return ((ClusterImporter.EncryptedState) encryptedState).decrypt(password);
        });
